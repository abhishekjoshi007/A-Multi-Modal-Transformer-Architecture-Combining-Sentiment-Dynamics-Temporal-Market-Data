...
embedding_file = os.path.join(output_dir, "node_embeddings.pt")
...
# Find top 3 most similar nodes (stocks)
top_similarities = []
for i in range(similarities.shape[0]):
    for j in range(i + 1, similarities.shape[0]):
        top_similarities.append((i, j, similarities[i, j]))

top_similarities = sorted(top_similarities, key=lambda x: x[2], reverse=True)[:3]

# Load the mapping of node indices to ticker names
# Assuming that you have some mapping function, e.g., `index_to_ticker_map`
# Example: index_to_ticker_map = {0: 'AAPL', 1: 'MSFT', ...}

# Print top 3 most similar stocks
print("Stock 1, Stock 2, Similarity Score")
for stock1, stock2, score in top_similarities:
    print(f"{stock1}, {stock2}, {score}")
